...
kbfong
PA1
List.java is a program that implements all the required functions for a linked list that was in the pdf. This is used by Lex.java for sorting.
...
public class List
{
	...
	boolean equals(List L)
	{
		...
		Node thatTempFront = L.front;        
		if (length == L.length) //check if the List and L are at least the same length
		 {
			while( isEqual == true && thisTempFront != null && thatTempFront != null) 
			{
				if (thisTempFront.data == thatTempFront.data) //if the data within each node is equivalent move on to the next node to check
				{
	...
	public String toString()
	{
		...
		while(temp != null)
		{
			stringList += temp.data + " "; //places the data from the Node into the empty string
